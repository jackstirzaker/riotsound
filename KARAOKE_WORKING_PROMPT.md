...
- 2026-05-05 fixed on current branch: `sound-supervisor` now registers API handlers, applies role state, and starts `SnapserverMonitor` before waiting for PulseAudio; `PulseAudioWrapper` reconnects indefinitely in the background.
- 2026-05-05 fixed on current branch: `multiroom-client` builds/uses a Snapcast client with `--player pulse` so received audio returns to `balena-sound.output`.
- 2026-05-05 fixed on current branch: `multiroom-server` and `multiroom-client` use host networking so the advertised Snapcast IP and TCP `1704` are reachable.
...
- Spotify/librespot cutoff investigation: current code uses `SOUND_MULTIROOM_BUFFER_MS` for the snapserver stream buffer and defaults it to 400 ms. `SOUND_STANDALONE_BUFFER_MS` is not read by the current code.
